<template>
  <nav class="navbar navbar-light bg-navbar fixed-top">
    <div class="container-fluid">
      ...
      <button v-if="logged" class="btn btn-success" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasMenu"><i class="fas fa-folder-minus"></i> {{ $t('button.menu') }}</button>
      <button v-else type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#joinnow">{{ $t('button.signup') }}</button>
    </div>
  </nav>
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasMenu" aria-labelledby="offcanvasMenuLabel">
    <div class="offcanvas-header bg-dark text-light">
      <span id="offcanvasMenuLabel" class="fs-3"><i class="fas fa-folder-minus"></i> {{ $t('button.menu') }}</span>
      <button type="button" class="btn-close btn-close-white text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body bg-index text-center">
      <p class="fs-4">{{ $t('navbar.offcanvas.title') }}</p>
      ...
      <router-link to="/surveys" class="btn btn-outline-dark" data-bs-dismiss="offcanvas"><i class="fas fa-home"></i> {{ $t('button.homepage') }}</router-link>
      <br>
      <router-link to="/profile" class="btn btn-outline-dark mt-3" data-bs-dismiss="offcanvas"><i class="fas fa-user-circle"></i> {{ $t('button.profile') }}</router-link>
      <br>
      <button @click="logout();" type="button" class="btn btn-outline-dark mt-3" data-bs-dismiss="offcanvas"><i class="fas fa-sign-out-alt"></i> {{ $t('button.logout') }}</button>
    </div>
  </div>
        ...
        <div class="modal-content text-center">
          <ul class="nav nav-tabs bg-dark " role="tablist">
            <li class="nav-item text-center"> 
              <p class="nav-link active btl" data-bs-toggle="tab" href="#login" role="tab">{{ $t('button.login') }}</p> 
            </li>
            <li class="nav-item text-center"> 
              <p class="nav-link btr" data-bs-toggle="tab" href="#register" role="tab">{{ $t('button.signup') }}</p> 
            </li>
          ...
          <div class="tab-content">
            <div class="tab-pane fade show active px-4 pt-3" id="login">
              ...
              <label class="form-label fw-bolder"><i class="fas fa-user-circle"></i> {{ $t('user.username') }}</label>
              ...
              <button @click="login()" class="btn btn-dark btn-block my-4 w-100 rounded">{{ $t('button.login') }}</button> 
            </div>
            <div class="tab-pane fade px-4 pt-3" id="register">
              ...
              <button @click="register()" class="btn btn-dark btn-block my-4 w-100 rounded">{{ $t('button.signup') }}</button> 
            </div>
          </div>
        </div>
      </div>
